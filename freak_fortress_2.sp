...
new bool:noSpeedMgmt[MAXSPECIALS];
...
public OnPluginStart()
{
	...
	AddCommandListener(DoMedic, "voicemenu"); 
	...
	RegAdminCmd("ff2_rage", Command_GiveRage, ADMFLAG_CHEATS, "Usage:  ff2_rage <boss>.  Give boss full rage.");
...
{			
	...
	// LOCAL TESTING ONLY
	noSpeedMgmt[Specials] = bool:KvGetNum(BossKV[Specials], "no_speed_management", 0);
...
public Action:checkItems(Handle:hTimer, any:client)  //Weapon balance 2
{
	...
	if(weapon && IsValidEdict(weapon))
	{
		...
		switch(index)
		{
			case 231:  //Razorback, Darwin's Danger Shield
			{
	...
	if(IsValidEntity(FindPlayerBack(client, {231}, 2)))  //Razorback, Darwin's Danger Shield
	{
		RemovePlayerBack(client, {231}, 2);
		weapon=SpawnWeapon(client, "tf_weapon_smg", 16, 1, 0, "");
	}
...
public Action:Command_GiveRage(client, args)
{
	FF2_SetBossCharge(0, 0, 100.0);
	FF2_SetBossCharge(1, 0, 100.0);
	FF2_SetBossCharge(2, 0, 100.0);
	
	PrintToConsole(client, "Gave boss rage.");
...
public Action:event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if((CheckRoundState()!=1 || !(FF2flags[client] & FF2FLAG_ALLOWSPAWNINBOSSTEAM)))
	{
		if(!(FF2flags[client] & FF2FLAG_HASONGIVED))
		{
			...
			RemovePlayerBack(client, {133, 231, 405, 444, 608, 642}, 7);
...
public Action:BossTimer(Handle:hTimer)
{
	...
	for(new client=0; client<=MaxClients; client++)
	{
		...
		if (!noSpeedMgmt[Special[client]]) // LOCAL ONLY
		...
		{		
			...
			if(KvJumpToKey(BossKV[Special[client]], ability))
			{
				...
				if(!ability[0])
				{
					...
					//PrintToServer("ability=%s / plugin=%s / client=%d / slot=%d / buttonmode=%d", ability_name, plugin_name, client, slot, buttonmode);
...
public Action:DoMedic(clientIdx, const String:command[], argc)
{
	new String:unparsedArgs[4];
	GetCmdArgString(unparsedArgs, 4);

	if (!strcmp(unparsedArgs, "0 0"))
		return DoTaunt(clientIdx, "taunt", argc);
	return Plugin_Continue;
}
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if(IsBoss(attacker))
	{
		if(IsValidClient(client) && !IsBoss(client) && !TF2_IsPlayerInCondition(client, TFCond_Bonked) && !TF2_IsPlayerInCondition(client, TFCond_Ubercharged))
		{
			...
			if(damage<=160.0)// && weapon != GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee))
			{
				damage*=3;
				return Plugin_Changed;
			}
		...
		if(boss!=-1)
		{
			if(attacker<=MaxClients)
			{
				...
				switch(wepindex)
				{
					...
					case 132, 266, 482, 1082, 172, 327, 404:  //Eyelander, HHHH, Nessie's Nine Iron, Festive Eyelander
					{
...
stock TF2_IsWearable(wearable)
{
	if(isWearable==INVALID_HANDLE)	
	{
		...
		if((isWearable=EndPrepSDKCall())==INVALID_HANDLE)
		{
